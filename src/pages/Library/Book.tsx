...
export const Book = (props: BookType['data'][number]) => {
    ...
    return <div className="flex flex-col gap-4">
        ...
        <div>
            ...
            <p className="font-light text-2xl">{props.attributes.employees.data.map(worker => <span className="text-[#12A1DD] cursor-pointer" onClick={() => navigate({to: '/cathedra/employees/$slug', params: {slug: worker.id.toString()}})}>{`${worker.attributes.last_name} ${worker.attributes.first_name.charAt(0)}. ${worker.attributes.middle_name?.charAt(0)}.`}, </span>)} {props.attributes.coworkers && props.attributes.coworkers.map(i => i.name).join(', ')}</p>
        </div>
        <div>
